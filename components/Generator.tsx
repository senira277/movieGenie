...
// Maps raw API/network error strings to safe, user-friendly messages
function getFriendlyError(raw: string = ""): string {
  const r = raw.toLowerCase();
  if (r.includes("billing") || r.includes("payment") || r.includes("quota") || r.includes("insufficient"))
    return "Service is temporarily unavailable. Please try again later.";
  if (r.includes("rate") || r.includes("limit") || r.includes("429"))
    return "Daily limit reached! Come back tomorrow.";
  if (r.includes("validation") || r.includes("strict"))
    return "No matches found. Try loosening the rating or time period.";
  if (r.includes("invalid") || r.includes("format") || r.includes("parse"))
    return "Something went wrong on our end. Please try again.";
  if (r.includes("network") || r.includes("fetch") || r.includes("failed to fetch"))
    return "Connection error. Please check your internet and retry.";
...
export default function Generator() {
  ...
  const handleGenerate = async (isJumble = false) => {
    ...
    try {
      ...
      if (!res.ok) {
        // Never expose raw API error messages — map to a safe token first
        const raw = data.message || "";
        throw new Error(
          raw.toLowerCase().includes("billing") || raw.toLowerCase().includes("quota")
            ? "billing"
        ...
        {isLoading ? (
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center pt-8 border-t border-gray-100">
              ...
              {/* Reddit Support Button */}
              <a
                href="https://www.reddit.com/r/GenieofMovie/"
                target="_blank"
                rel="noopener noreferrer"
                className="px-6 py-3 rounded-xl bg-[#FF4500] text-white font-semibold hover:bg-[#e03d00] shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2"
              >
                Follow on Reddit
              </a>
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-3">
                ...
                {errorMsg && (
                  <p className="text-red-500 text-center text-sm font-medium animate-in fade-in slide-in-from-top-1 flex items-center justify-center gap-1.5">
                    <span>⚠</span> {errorMsg}
                  </p>
                )}
              </div>
